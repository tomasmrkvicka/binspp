...
KumulaVsech <- function(CC, omega, x_left, x_right, y_bottom, y_top) {
  ...
  for (i in 1:length(x_left)) {
    ...
    .dist <- function(C) {
      pmvnorm(lower = c(x_left[i], y_bottom[i]), upper = c(x_right[i], y_top[i]), mean = C, sigma = diag(2) * omega^2)
    }
...
GenerateCenterPoint <- function(W_dil) {
  P <- runifpoint(1, W_dil)
  c(P$x[1], P$y[1])
}
...
StepMovePoint <- function(kappa, alpha, omega, theta, r, likelihoodprev, rho0sum, X, CC, logP, integral, AreaMRW, W_dil,
  ...
  if (runif(1) < 1 / 3) {
    ...
    if (log(runif(1)) < (logP2 - logP + likelihood - likelihoodprev)) {
      Vystup <- list(CC1, logP2, int2, likelihood, rhosum)
    } else {
      Vystup <- list(CC, logP, integral, likelihoodprev, rho0sum)
    }
  } else {
...
#' CC.true <- AuxVarGen(
#'   kappa, c(theta1, theta2), likelihoodprev, rho0sum, CC,
#'   AreaMRW, W_dil, 100
#' )
#'
...
#' control <- list(
#'   NStep = 50, BurnIn = 25, SamplingFreq = 10,
...
estinternsp <- function(X, control,
  ...
  for (step in 2:NStep) {
    ...
    # update likelihoods
    if (Newalpha < alpha_lb | Newalpha > alpha_ub | Newomega < omega_lb | Newomega > omega_ub) {
      logratio <- -Inf
    } else {
    ...
    if (Newkappa < kappa_lb | Newkappa > kappa_ub | Newtheta1 < theta1_lb | Newtheta1 > theta1_ub | Newtheta2 < theta2_lb | Newtheta2 > theta2_ub) {
      logratio <- -Inf
    } else {
      # generate auxiliary variable for given proposed parameters
    ...
    r <- coeff(c(theta1, theta2))
    BDMCMCres <- StepMovePoint(
      kappa, alpha, omega, c(theta1, theta2), r, likelihoodprev, rho0sum, X, CC, logP, integral, AreaMRW, W_dil,
      x_left, x_right, y_bottom, y_top,
      AreaW
    )
    CC <- BDMCMCres[[1]]
