...
estgtp <- function(X,
  ...
  X$P <- X$P - 1
  C$mus <- intfun_cols_cpp(
    cbind(C$x, C$y), omega0, X$xrange,
    X$yrange
  )
  ...
  for (i in 2:iter) {
    ...
    ktmp <- kappas[i - 1]
    omegas[i] <- otmp
    omegas[i] <- update_omega(
      otmp, somega, X, C, ltmp, ttmp,
      dgp, a_omega, b_omega
    )
    upd_lt <- update_lt(
      ltmp, ttmp, stheta, dlambda, X, C,
      otmp, dgp, l_lambda, u_lambda, a_theta, b_theta
    )
    lambdas[i] <- upd_lt[1]
    thetas[i] <- upd_lt[2]
    dgp <- dgp_comp_cpp(lambdas[i], thetas[i])
    kappas[i] <- update_kappa(
      ktmp, skappa, length(C$x),
      Cwin_ar, a_kappa, b_kappa
    )
    if (n_upd > 0) {
      switch <- update_P_cpp_n(
        X, C, otmp, ltmp, ttmp, C$mus,
        dgp, n_upd, switch
      )
    }
    sbirth <- smove
    if (n_bdm > 0) {
      ...
      for (j in 1:n_bdm) {
        if (R[j] <= 0.5) {
          move <- move_C_cpp(
            X, C, otmp, ltmp, ttmp, smove,
            dgp, move
          )
        }
        if ((R[j] > 0.5) & (R[j] <= 0.75)) {
          birth_count <- c_birth_cpp(
            X, C, ktmp, otmp,
            ltmp, ttmp, sbirth, dgp, birth_count
          )
        }
        if (R[j] > 0.75) {
          death_count <- c_death_cpp(
            X, C, ktmp, otmp,
    ...
    if (i %% plot.step == 0) {
      ...
      plot(n_Ciw[1:(i - 1)], type = "l", ylim = range(c(
        n_Ciw[1:i],
        n_Cow[1:i]
      )))
      ...
      unique(n1 <- sapply(1:length(C$x), function(k) {
        sum(X$P ==
          k)
      }))
      mus.end <- intfun_cols_cpp(
        cbind(C$x, C$y), omega0,
        c(0, 1), c(0, 1)
      )
      in_win <- which(mus.end >= 0.95)
      barplot((table(c(n1[in_win], 0:8)) - 1) / sum(table(c(
        n1[in_win],
        0:8
      )) - 1), ylim = c(0, 0.5))
    }
    ...
    if (i %% save.step == 0) {
      saveRDS(object = list(C = C, X = X, C0 = C0, pars = list(
        kappas,
        omegas, lambdas, thetas
      )), filename)
    }
  }
  res <- list(
    kappa = kappas, omega = omegas, lambda = lambdas,
    theta = thetas, Clist = Clist, Plist = Plist, n_C = n_C,
    n_Ciw = n_Ciw, n_Cow = n_Cow, info = c(
      birth = birth_count,
