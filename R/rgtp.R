...
rgtp <- function(kappa, omega, lambda, theta,
  ...
  if (is.null(C)) {
    ...
    C <- lapply(1:nsim, function(x) {
      cbind(runif(Nc[x], win$xrange[1] -
  ...
  daught <- lapply(1:length(C), function(k) {
    do.call(
      rbind,
      lapply(1:length(N_daught[[k]]), function(z) {
        cbind(rnorm(
          N_daught[[k]][z],
          C[[k]][z, 1], omega
  ...
  for (i in 1:length(C)) {
    ...
    P[[i]] <- P[[i]][which((daught[[i]][, 1] >= win$xrange[1]) &
      ...
      ,
      2
    ] >= win$yrange[1]) & (daught[[i]][, 2] <= win$yrange[2]))]
  }
