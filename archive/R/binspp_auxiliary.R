...
#'
#' @description This function provides means to simulate realizations from the Thomas-type cluster point process with complex inhomogeneities.
#'
#' @param kappa intensity or intensity function of the parent process, scalar or pixel image in the “im” format of the “spatstat” package.
...
#' @param W the observation window where the realization is to be generated, in the “owin” format of the “spatstat” package.
...
#' @param betavec vector of parameters describing the dependence of the intensity function of the parent process on covariates in the list z_beta.
#' @param alphavec vector of parameters describing the dependence of the mean number of points in a cluster on covariates in the list z_alpha.
#' @param omegavec vector of parameters describing the dependence of the spread of the clusters on covariates in the list z_omega.
#' @param z_beta list of covariates describing the intensity function of the parent process, each covariate being a pixel image as used in the “spatstat” package.
#' @param z_alpha list of covariates describing the location-dependent mean number of points in a cluster, each covariate being a pixel image as used in the “spatstat” package.
#' @param z_omega list of covariates describing the location-dependent scale of a cluster, each covariate being a pixel image as used in the “spatstat” package.
#'
#' @return A planar point pattern, object of the type “ppp” used in the “spatstat” package.
#'
#' @examples
#'
...
#'
#' @export
rThomasInhom <- function(kappa, alpha, omega, W, W_dil, betavec=NULL, alphavec=NULL, omegavec=NULL, z_beta=NULL, z_alpha=NULL, z_omega=NULL){
...
#'
#' @description For exploratory purposes it may be useful to perform the first step of the analysis only, to investigate the dependence of the intensity function of the parent process on given covariates, without running the MCMC chain.
#'
#' @param X observed point pattern in the “ppp” format of the “spatstat” package.
#' @param z_beta list of covariates describing the intensity function of the parent process, each covariate being a pixel image as used in the “spatstat” package.
...
#'
#' @return List containing the output of the “ppm” function from the “spatstat” package, along with some auxiliary objects useful for running the MCMC chain.
#'
#' @examples
#'
#' # Prepare the dataset:
...
#'
#' @export
first.step <- function(X, z_beta, W_dil, plot=TRUE){
  if (length(z_beta)>0){
    ...
    if (plot){plot(exp(FS$coef[1])*B, main="Plot of estimated 1st-order intensity function")}
    ...
    # Estimated intensity function
    B = as.im(1, W=W_dil)
    if (plot){plot(exp(FS$coef)*B, main="Plot of estimated 1st-order intensity function")}
...
# @TODO
pbetXC = function(Y, z_beta, z_alpha, z_omega, NStep, SamplingFreq, x_left, x_right,y_bottom, y_top, output.first.step, control, AreaW, AreaMRW, W_dil, Wpix){
  ...
  lambda = exp(output.first.step$FS$coef[1]);
  kappa = max(lambda/exp(alpha),0.00001);
  CC = rpoispp(kappa*output.first.step$B);
  CC = t(rbind(CC$x,CC$y))

  integralrho = output.first.step$integralrho
  ...
  if (length(z_beta)>0){
    pBX_0 = c(kappa, alpha, alphabet, omega, omegabet, logP, integral, rep(1,length(output.first.step$FS$coef[-1])), 0, 0, 0, 0, 0, 0);
  } else {
  ...
  for(step in 1:NStep){
    ...
    S = StepbetC(kappa, z_alpha, z_omega, alpha, salpha, alphabet, salphabet, omega, somega, omegabet, somegabet,
                 Y, CC, logP, integral, integralrho, x_left, x_right, y_bottom, y_top, Wpix, AreaW, output.first.step$FS$coef[1],
    ...
    NewCenter = NewCenterPoint(W_dil,output.first.step$B)
    ...
    if( (step %% SamplingFreq) == 0){
      if (length(z_beta)>0){
        ...
        pv = rep(0,length(output.first.step$FS$coef[-1]))
...
#' @param z_omega list of covariates describing the location-dependent scale of a cluster, each covariate being a pixel image as used in the “spatstat” package.
#'
#' @return The output of the function is given by the list containing the parameter estimates along with the 2.5% and 97.5% quantiles of the posterior distributions. Also, several auxiliary objects are included in the list which are needed for the “print.outputs” and “plot.outputs” functions.
#'
#' @examples
#'
#' # Prepare the dataset:
...
#'
#' @export
binspp = function(X, control, x_left, x_right, y_bottom, y_top, W_dil, z_beta=NULL, z_alpha=NULL, z_omega=NULL){
  ...
  fW <- function(x,y){as.numeric(inside.owin(x,y,W))}
  ...
  fWMR <- function(x,y){as.numeric(inside.owin(x,y,W_dil))}
  ...
  output.first.step = first.step(X,z_beta,W_dil,plot=FALSE)

  pBX_out = pbetXC(Y, z_beta, z_alpha, z_omega, NStep, SamplingFreq, x_left, x_right, y_bottom, y_top, output.first.step, control, AreaW, AreaMRW, W_dil, Wpix)
  pBX = pBX_out$pBX
  ...
  if(length(z_alpha)>0) AlphabetCI = apply(Postalphabet,2,Q) else AlphabetCI = NULL
  OmegaCI = quantile(Postomega, probs = c(0.025,0.975))
  if(length(z_omega)>0) OmegabetCI = apply(Postomegabet,2,Q) else OmegabetCI = NULL
  if(length(z_beta)>0) BetaCI = apply(Postbeta,2,Q) else BetaCI = NULL
  ...
  res = list(Kappahat=Kappahat,
             ...
             Betahat=Betahat,
             BetaCI=BetaCI,
             bet=output.first.step$FS$coef[-1],
             first.step=output.first.step,
...
#'
#' @param Output list, output of the main function binspp.
#' @param BurnIn new value of burn-in.
#'
#' @return List containing the parameter estimates along with the 2.5% and 97.5% quantiles of the posterior distributions, along with auxiliary objects needed for printing and plotting the outputs.
#'
#' @examples
#'
#' # Prepare the dataset:
...
#'
#' @description This function provides the summaries of the posterior distributions in the text form.
#'
#' @param Output list, output of the main function binspp.
#'
...
print.outputs <- function(Output){
  ...
  if (length(Output$z_omega)>0){
    ...
    cat(Output$first.step$FS$coef[1])
    cat("\n")
...
#'
#' @description This function produces a series of plots providing a graphical representation of the posterior distributions in terms of histograms and trace plots.
#'
#' @param Output list, output of the main function binspp.
#'
#' @return Series of plots providing a graphical representation of the posterior distributions in terms of histograms and trace plots.
#'
#' @examples
#'
#' # Prepare the dataset:
...
plot.outputs <- function(Output){
  ...
  if (length(Output$z_beta)>0){
    ...
    B = exp(B) #Covariate part of intensity
    plot(exp(Output$first.step$FS$coef[1])*B, main="Plot of estimated first-order intensity function")
  }

  # Plotting alpha(u):
  if (Output$noAlpha>0){
    fWMR <- function(x,y){as.numeric(inside.owin(x,y,Output$W_dil))}
